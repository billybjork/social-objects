defmodule SocialObjects.Workers.TiktokSyncWorker do
  @moduledoc """
  Oban worker that syncs TikTok Shop product catalog to the database.
  ...
  alias SocialObjects.Catalog.SizeExtractor
  ...
  defp find_matching_product(brand_id, tiktok_skus) do
    ...
    if Enum.empty?(seller_skus) do
      ...
      variant =
        from(v in SocialObjects.Catalog.ProductVariant,
  ...
  defp replace_tiktok_images(product, tiktok_product_data) do
    ...
    if Enum.any?(image_records) do
      {count, _} = Repo.insert_all(SocialObjects.Catalog.ProductImage, image_records)
  ...
  defp has_primary_image?(product_id) do
    from(pi in SocialObjects.Catalog.ProductImage,
  ...
  defp extract_size_attrs_for_variant(variant, sales_attributes) do
    ...
    product_name =
      from(p in SocialObjects.Catalog.Product, where: p.id == ^variant.product_id, select: p.name)
      |> Repo.one()
